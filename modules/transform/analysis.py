...
class AnalysisTransform_prompt(nn.Module):
    def __init__(self, N, M):
        super().__init__()
        self.analysis_transform = nn.ModuleList([
    ...
    def forward(self, x, prompt_proj, prompt):
        count = 0
        mu = []
        log_var = []
        mu, log_var, P = prompt
        for i in range(3):
            x = self.analysis_transform[count](x)
            x, _, _, _= prompt_proj[i](x, mu, log_var)
            count += 2
        x = self.analysis_transform[count](x)
        # x, _, _, _= prompt_proj[3](x, mu, log_var)
