...
def test_clip_one_epoch(epoch, test_dataloader, model, criterion, save_dir, logger_val, tb_logger, clip_loss, lambda_clip):
    model.eval()
    device = next(model.parameters()).device
    ...
    with torch.no_grad():
        for i, d in enumerate(test_dataloader):
            ...
            out_net = model(d)
            out_criterion = criterion(out_net, d)
            closs = clip_loss(out_net["x_hat"], d, "eval")
            aux_loss.update(model.aux_loss())
            bpp_loss.update(out_criterion["bpp_loss"])
            cliploss.update(closs)
            loss.update(out_criterion["loss"]+(lambda_clip * closs))
            if out_criterion["mse_loss"] is not None:
                mse_loss.update(out_criterion["mse_loss"])
            if out_criterion["ms_ssim_loss"] is not None:
                ms_ssim_loss.update(out_criterion["ms_ssim_loss"])
...
def test_conditional_one_epoch(epoch, test_dataloader, model, criterion, save_dir, logger_val, tb_logger, clip_loss, lambda_clip):
    model.eval()
    device = next(model.parameters()).device
...
def newtest_conditional_one_epoch(epoch, test_dataloader, model, criterion, save_dir, logger_val, tb_logger, clip_loss, lambda_clip):
    model.eval()
    device = next(model.parameters()).device
    ...
    with torch.no_grad():
        for i, d in enumerate(test_dataloader):
            ...
            out_net = model((d, betas))
            out_criterion = criterion(out_net, d)
            clip_out = clip_loss(out_net["x_hat"], d, "eval")
            closs = clip_out["fc"] + clip_out["clip_conv_all"]
            closs = closs.mean()
            aux_loss.update(model.aux_loss())
            bpp_loss.update(out_criterion["bpp_loss"])
            cliploss.update(closs)
